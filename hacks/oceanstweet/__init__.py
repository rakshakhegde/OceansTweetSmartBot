...
from hacks.oceanstweet.config import *

app = flask.Blueprint('app', __name__, template_folder='templates')

app_url = 'http://139.59.1.201:5000/oceanstweet/'
...
def newCall():
	if cidExists(cid):
		...
		ET.SubElement(response, 'playtext').text = 'Welcome to Oceans Tweet. Press hash to stop recording.'
		import time, random
		ET.SubElement(response, 'record', {'maxduration': '120'}).text = str(time.time()) + str(random.random())
		ET.SubElement(response, 'playtext').text = 'Oceans Tweet posting'
	else:
		response = ET.Element('response')
		ET.SubElement(response, 'playtext').text = 'Welcome to Oceans Tweet Bot, a Digital Ocean Social Media Challenge demo. Please log in to Twitter by clicking on your personal secret link sent to you via SMS. Created by Ruck Shuck R Heg day.'
		ET.SubElement(response, 'hangup')
